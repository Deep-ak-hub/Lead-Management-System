...
const Status = () => {
  ...
  const {
    ...
    {
      page,
      inquiry: status == "followup" ? filter : null,
      date: status == "followup" ? date : null,
      status:
        status == "followup"
          ? "Follow Up"
  ...
  if (
    ...
    (admin?.data?.purchasedPlan != "010" || admin?.data?.purchasedPlan != "011")
  ) {
    return (
      <p className="text-xl text-center text-secondary">
        Cannot access this plan
      </p>
    );
  }
    ...
    <div className="relative px-4 my-8 flex flex-col w-full rounded-lg">
      <div>
        <div className="flex items-center gap-3">
          ...
          {status == "followup" && (
            <div className="flex text-white items-center gap-3">
              <button
                className={`cursor-pointer border px-4 py-1 rounded-full ${filter == "project" ? "bg-accent text-white" : "border-accent text-accent bg-white"}`}
                onClick={() => {
              ...
              <button
                className={`cursor-pointer border px-4 py-1 rounded-full ${filter == "course" ? "bg-accent text-white" : "border-accent text-accent bg-white"}`}
                onClick={() => {
              ...
              <button
                className={`cursor-pointer border px-4 py-1 rounded-full   ${date ? "bg-accent text-white" : "border-accent text-accent bg-white"}`}
                onClick={() => {
          ...
          <table className="table-auto border-none border-gray-400 p-2 w-full my-6 rounded-lg ">
            <thead className="sticky top-0 bg-accent">
              <tr>
                ...
                <th className="hidden md:table-cell border border-gray-300 p-2">
                  Email
                </th>
                <th className="border border-gray-300 p-2">Phone</th>
                <th className="hidden md:table-cell border border-gray-300 p-2">
                  {status == "followup" ? "Follow up" : "Created At"}
                </th>
                ...
                return (
                  <tr key={items._id} className="text-center">
                    ...
                    <td className="border border-gray-300 text-accent font-medium p-2">
                      <Link to={`../lead/${items._id}`}>{items.name}</Link>
                    </td>
                    ...
                    <td className="hidden md:table-cell border border-gray-300 p-2">
                      {status == "followup"
                        ? items.followUp
                          ? new Date(items.followUp).toLocaleDateString(
                              "en-Np",
                              {
                    ...
                    <td className="p-2 border text-center border-slate-200 w-fit">
                      <div className="flex justify-center gap-4">
                        <button
                          onClick={() =>
                            setSelectedValue({ user: items, action: "edit" })
                          }
                          className="text-accent hover:bg-accent/10 p-2 rounded"
                        >
                          <FaEdit className="" />
                        </button>
                        <button
                          onClick={() =>
                            setSelectedValue({ user: items, action: "delete" })
                          }
        ...
        <Modal
          ...
          className={
            "fixed top-1/2 left-1/2 -translate-1/2 w-full md:w-1/2 bg-white/5 p-5 rounded-lg text-white backdrop-blur-2xl"
          }
        >
          <UpdateForm
        ...
        <Modal
          ...
          className={
            "min-w-90 w-1/2 bg-white/5 text-white backdrop-blur-2xl p-4 fixed top-1/2 left-1/2 -translate-1/2 rounded-xl shadow-xl"
          }
        >
          <h2 className="font-semibold mb-3">
            Are you sure to delete{" "}
            <span className="text-accent">{selectedValue?.user?.name}</span>?
          </h2>
          ...
          <div className="flex gap-3 items-center mt-3">
            <button
              onClick={() => handleDelete(selectedValue?.user?._id)}
              className="cursor-pointer px-4 py-2 bg-red-500 text-white rounded font-medium"
            >
              Delete
            </button>
            <button
              onClick={() => setSelectedValue(null)}
              className="cursor-pointer bg-gray-500 text-white px-3 py-2 rounded"
            >
              Cancel
            </button>
          </div>
        ...
        {!isFetching && leads?.data?.leads?.length == 0 && (
          <p className="text-center font-bold text-3xl my-6 text-gray-500">
            No Records Found
          </p>
        )}
        {isFetching && <Loading />}
        {leads?.data?.leads?.length > 0 && (
          <div className="flex justify-end items-center text-white gap-3 my-4">
            <p>
              {page} / {leads?.data?.pagination?.totalPage}
            </p>
            <button
