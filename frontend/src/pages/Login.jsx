...
import { useResetPasswordMutation, useSigninAdminMutation } from '../redux/Api/Admin'
...
const Login = () => {
    ...
    const [resetPassword, { error, isLoading: isLoadingReset }] = useResetPasswordMutation()
        ...
        <>
            <div className='min-h-screen flex items-center justify-center bg-black px-4'>
                <div className='bg-white/5 shadow-lg rounded-lg p-8 w-full max-w-2xl'>
                    <h1 className='text-3xl font-bold text-center text-gray-300 mb-6 underline underline-offset-8'>Admin Login</h1>
                    <form onSubmit={handleSubmit} className='space-y-5 rounded-lg'>
                        <div className='grid grid-cols-1 md:grid-cols-1 gap-4'>
                            ...
                            <div className='flex flex-col'>
                                <label htmlFor="password" className='text-lg font-semibold text-gray-200'>Password</label>
                                <div
                                    className='bg-white border border-gray-300 rounded-lg p-2 focus:ring focus:ring-primary outline-none my-2 flex items-center justify-between'>
                                    <input
                        ...
                        <button onClick={handleForgotPassword} className='text-white hover:text-primary cursor-pointer underline underline-offset-2'>
                            Forgot Password
                        </button>
                    </form>
                </div>
            ...
            <Modal isOpen={isModalOpen} onRequestClose={() => setIsModalOpen(false)}
                ...
                <div className='flex justify-start gap-2 mt-4'>
                    <button onClick={handleSendEmail}
                        ...
                        className='bg-primary disabled:bg-gray-500 text-white px-4 py-2 rounded cursor-pointer'>{isLoadingReset ? "Loading..." : "Send Email"}</button>
                    <button onClick={() => {
                        setIsModalOpen(false)
                        setErrorMessage('')
                    }} className='bg-gray-500 cursor-pointer text-white px-4 py-2 rounded hover:bg-gray-500'>Cancel</button>
