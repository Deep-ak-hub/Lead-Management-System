        ...
        <>
            <div className="overflow-y-auto px-6 flex uppercase flex-col gap-6">
                ...
                <div>
                    <span className="text-white flex items-center flex-row gap-3 font-bold text-xl">
                        <FaUsers />Manage Leads
                    </span>
                    <div className="mt-3 flex gap-2 flex-col">
                        {status.map((el, index) => (<NavLink
                            key={el.label}
                            onClick={() => setTimeout(() => setIsOpen(false), 0)}
                            to={el.to}
                            className={({ isActive }) =>
                                `hover:text-accent flex items-center gap-2 p-1 transition-all duration-300 ${isActive ? "bg-accent/10 rounded-r-lg text-accent font-semibold border-l-4 px-2" : "text-white"
                                }`
                            }
                        >
                            {el.icon}{el.label}
                ...
                <NavLink
                    ...
                    onClick={() => setTimeout(() => setIsOpen(false), 0)}
                    className={({ isActive }) =>
                        `pl-1 flex text-xl items-center gap-3 hover:text-accent transition font-semibold duration-300 ${isActive ? "text-accent " : "text-white"
                        }`}
                >
                    <IoMdSettings />
                    Services
            ...
            <div className="flex gap-2 px-3 pt-2 border-t border-white/30 items-center">
                ...
                <div className="flex flex-col items-start text-left">
                    <Link onClick={() => setTimeout(() => setIsOpen(false), 0)} to={admin?.data?.role == 'superadmin' ? 'profile/create-admin' : '/not-found'}>
                        <p className="max-w-28 text-white truncate line-clamp-1">{admin?.data?.email}</p>
